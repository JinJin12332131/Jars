...
class Fusion(BaseModel):
    ...
    def modify_commandline_options(parser, isTrain=True):
        ...
        if isTrain:
            ...
            parser.add_argument('--lambda_B', type=float, default=1.0, help='weight for msi_msi and msi_s_lr   beta')
            #parser.add_argument('--lambda_C', type=float, default=1.0, help='weight for msi_s_lr   alpha')
            parser.add_argument('--lambda_D', type=float, default=1.0, help='weight for sum2one and sparse   mu')
            ...
            parser.add_argument('--l_h', type=int, default=128)
            parser.add_argument('--h_h', type=int, default=128)
    ...
    def initialize(self, opt, hsi_channels, msi_channels, sp_matrix, sp_range):
        ...
        self.net_MSI2S = network.define_msi2s(l_h=l_h,h_h=h_h,n_heads=n_heads,input_ch=msi_channels, output_ch=num_s, gpu_ids=self.gpu_ids,
        ...
        #self.net_m2img = network.define_s2img(input_ch=num_s, output_ch=msi_channels, gpu_ids=self.gpu_ids)

        # encoder for hsi
        self.net_LR2s = network.define_lr2s(l_h=l_h, n_heads=n_heads,input_ch=hsi_channels, output_ch=num_s, gpu_ids=self.gpu_ids,
                                                useSoftmax=opt.useSoftmax)
        # define psf function
    ...
    def forward(self):
        # first lr process
        self.rec_lr_s , self.out_f = self.net_LR2s(self.real_lhsi)
        self.rec_lr_lr = self.net_s2img(self.rec_lr_s)

        #second msi process
        self.rec_msi_s = self.net_MSI2S(self.real_hmsi,self.out_f)
        #self.rec_msi= self.net_m2img(self.rec_msi_s)
        self.rec_msi_hr = self.net_s2img(self.rec_msi_s)

        #estimate psf and srf
        self.rec_msi_msi = self.net_G_HR2MSI(self.rec_msi_hr)#SRF
        self.rec_msi_lrs = self.net_PSF(self.rec_msi_hr)#PSF
        #self.rec_msi_lrs = self.net_PSF(self.rec_msi_s)#PSF
    ...
    def backward_joint(self, epoch):
        ...
        self.loss_lr_sparse = self.criterionSparse(self.rec_lr_s) * self.opt.lambda_D

        #self.loss_hrmsi_pixelwise = self.criterionPixelwise(self.real_hmsi, self.rec_msi) * self.opt.lambda_A
        ...
        self.loss_msi_sparse = self.criterionSparse(self.rec_msi_s) * self.opt.lambda_D
        self.loss_msi = self.loss_msi_pixelwise + self.loss_msi_s_sumtoone + self.loss_msi_sparse
        # PSF
        self.loss_msi_ss_lr =  self.criterionPixelwise(self.real_lhsi, self.rec_msi_lrs) * self.opt.lambda_B
        # lrmsi
